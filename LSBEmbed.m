function [mse,p,s,er ] = LSBEmbed(fullcoverpath,coverfilename,watermark )
...
fileName='untexturedcover.tif';%untexturedcover.tif,seminontexture512.jpg,mandrill512.tiff,wash-ir512.tif,untexturedcover.png,boat.512.tiff,wash-ir.tiff,textured512.tiff
originalImage = imread(fullcoverpath);
...
messageFileName='blacktree50.gif'; %blacktree50.gif,desert100.gif,lena150.png,blacktree200.gif,plot250.gif,man256.gif,lena300.gif,lena350.jpg,300.gif,,lena128.gif,facebook.jpg,SpiralEmbedding.jpg,red240.png,lenafetch.png,pepper512.gif,faces.gif,dtcx64.gif',boat512plot2.png,usinex128.gif,skull.gif
hiddenImage = imread(watermark);
...
[s,map] = ssim(double(originalImage),double(stego));
er= embeddingrate;

result= strcat('S-Histogram:','MSE=',num2str(mse),',PSNR=',num2str(p),',SSIM=',num2str(s),', Bpp=',num2str(embeddingrate));
...
subplot(3, 3, 1);
imshow(hiddenImage, []);
title('Watermark Image', 'FontSize', fontSize);

% Display the original gray scale image.
subplot(3, 3, 2);
imshow(originalImage, []);
title('Cover Image', 'FontSize', fontSize);
...
subplot(3, 3, 4); 
bar(pixelCount);
title('Watermark Histogram', 'FontSize', fontSize);
...
subplot(3, 3, 5); 
bar(pixelCount);
title('Cover Histogram', 'FontSize', fontSize);
xlim([0 grayLevels(end)]); % Scale x axis manually.
grid on;
...
% Save entire plot
filename = strcat(num2str(er),coverfilename);
saveas(gcf,filename)
